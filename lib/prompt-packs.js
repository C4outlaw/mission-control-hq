...
// One entry per prompt-pack product. Buyers browse the carousel and choose the
// system or artist they care about, so every entry carries its own cover and file.
export const PACKS = [
  {
    id: 'story-gravity',
    subject: 'Story Gravity',
    name: 'The Story Gravity Blueprint — Lost Jamaican Artist Edition',
    tagline: 'Research. Hook. Escalate. Pay Off.',
    cover: '/prompts/covers/story-gravity-v1.jpg',
    file: 'THE-STORY-GRAVITY-BLUEPRINT-CUSTOMER-PACKAGE-V1.zip',
    metric: '16 prompts',
    blurb:
      'MyrieHQ’s complete research-and-retention system for turning verified facts into artist documentaries, biographies, cultural profiles and serialized stories people want to finish.',
    includes: [
      'The 42-page Story Gravity Blueprint playbook and blank worksheets',
      'Sixteen copy-and-paste prompts for research, hooks, scripting and quality control',
      'The beat-level retention matrix that gives every 2.5–3.5 seconds a job',
      'Single-video and two-part serial builders plus the dead-middle repair system',
      'Narration direction, Visual Gravity handoff and analytics learning loop',
    ],
  },
  {
    id: 'spice',
  ...
  {
    id: 'squash-video-build',
    subject: 'Squash Video Build',
    name: 'Squash Video Build — Prompt Pack',
    tagline: 'The exact cinematic system behind the Squash film',
    cover: '/prompts/covers/squash-video-build-v1.jpg',
    file: 'SQUASH-VIDEO-BUILD.pdf',
    metric: '8 modules',
    blurb:
      'The complete production system behind The Lost Jamaican’s Squash film: the 34-phrase script architecture, the lens-spec shot direction method, the Interstellar grade recipe, the locked narrator voice settings, the assembly and loudness specs, and the trademark thumbnail layout — every prompt copy-paste ready.',
  },
];
...
// Fail closed: with the source public, an empty fallback key would let anyone
// forge download grants. No secret configured means no grants sign or verify.
const secret = () => {
  const s = process.env.PROMPT_PACK_SECRET;
  if (!s) throw new Error('PROMPT_PACK_SECRET is not configured');
  return s;
};
...
export function verifyGrant(token) {
  ...
  try {
    ...
    return null; // unconfigured secret can never validate a grant
  }
  const [raw, sig] = token.split('.');
  const expect = crypto.createHmac('sha256', key).update(raw).digest('base64url');
