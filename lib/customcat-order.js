...
export async function createCustomCatOrder(session, opts = {}) {
  ...
  // Stripe hands back whatever the buyer typed, usually "123 Main St Apt 4"
  // on one line. CustomCat verifies addresses on live orders (a listed 400
  // cause), and verifiers want the unit on its own line, so lift a trailing
  // apt / unit / suite / # off line 1 when line 2 is empty.
  let line1 = String(ship.line1 || '').trim();
  let line2 = String(ship.line2 || '').trim();
  if (!line2) {
    const m = line1.match(/^(.*\S)\s+((?:apt|apartment|unit|suite|ste|#)\.?\s*\S+)$/i);
