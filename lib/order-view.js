...
export function orderStatus(session) {
  ...
  if (stamp.tracking_number) {
    return {
      state: 'shipped',
      label: 'Shipped',
      detail: stamp.shipment && stamp.shipment.startsWith('partial')
        ? 'Part of your order has left the print house; the rest follows separately.'
        : 'Your order has left the print house.',
      tracking: stamp.tracking_number,
      trackingUrl: stamp.tracking_url || null,
      shippedAt: stamp.shipped_at || null,
      ref: stamp.cc_order_id || stamp.printify_order_id || null,
    };
  }
