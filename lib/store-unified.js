...
/* The store carries S, M, L and XL only. Mugs keep their single size.
   Compared lower-case: CustomCat writes "One Size" where this list read
   "One size", which quietly made every mug unsellable. */
const ALLOWED_SIZES = new Set([
  's', 'small', 'm', 'medium', 'l', 'large', 'xl', 'x-large', 'one size', '11oz',
]);
export const isStockedSize = (sz) => ALLOWED_SIZES.has(String(sz).toLowerCase());
...
// The premium line leads, straight under the masthead: it is the best-looking
// work in the shop and it carries the Jamaican wording, so it is what should
// meet a first-time visitor. The rest of the island line follows, then the
// everyday sellers with their model photography, and souvenirs close the page.
const DEPT = {
  premium: { id: 'premium', label: 'Premium', order: 1 },
  island: { id: 'island', label: 'The Island Line', order: 2 },
  best: { id: 'best', label: 'Best Sellers', order: 3 },
...
// No mug exists among the products this CustomCat account can order (checked
// against all 177 of them), so a mug order would be paid for and then refused.
// They stay out of the shop until a mug supplier is wired up.
const everyday = CATALOG30.filter((p) => p.kind !== 'mug').map((p) => {
...
const island = PRODUCTS.filter((p) => p.kind !== 'mug').map((p) => ({
  id: p.key,
  source: 'printify',
...
/* ---- source 3: the premium line, printed by CustomCat on the heavy blanks. ---- */
const premium = PREMIUM_PRODUCTS.map((p) => {
  ...
  return {
    ...
    blank,
    designKey: p.key,
    // Sellable only once real print art is mapped for this exact piece, so a
    // piece we cannot fulfil says "coming soon" instead of taking money.
    sellable: ccSellable(p.key, blank),
    source: 'customcat',
  };
});

const stocked = new Set([...island, ...premium, ...everyday].map((i) => i.dept));
export const DEPARTMENTS = Object.values(DEPT)
  .filter((d) => stocked.has(d.id))
  .sort((a, b) => a.order - b.order);

/** Premium first, then the island line, the everyday sellers, and souvenirs. */
export const CATALOGUE = [...island, ...premium, ...everyday].sort(
