...
const premium = PREMIUM_PRODUCTS.map((p) => {
  ...
  return {
    ...
    blank,
    designKey: p.key,
    // Sellable only once real print art is mapped for this exact piece. The
    // white colourways and the two non-typographic designs have none, so they
    // still say "coming soon" rather than taking money we cannot fulfil.
    sellable: ccSellable(p.key, blank),
    source: 'customcat',
  };
});
